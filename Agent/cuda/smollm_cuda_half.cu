...
__global__ void attn(const __half*qkv,const __half*kcache,const __half*vcache,__half*out,int layer,int pos){
  int h=blockIdx.x,tid=threadIdx.x,kv=h/3; __shared__ float scores[CTX],red[128]; float s=-1e20f;
  if(tid<CTX){
    if(tid<=pos){
      float acc=0; const __half*q=qkv+h*HD; const __half*k=&kcache[((layer*KVH+kv)*CTX+tid)*HD];
      #pragma unroll
      for(int d=0;d<HD;++d) acc+=__half2float(q[d])*__half2float(k[d]);
      s=acc*0.125f;
    }
    scores[tid]=s;
  }
  __syncthreads(); float mx=-1e20f; for(int t=tid;t<CTX;t+=128)mx=fmaxf(mx,scores[t]); red[tid]=mx; __syncthreads();
  for(int r=64;r;r>>=1){if(tid<r)red[tid]=fmaxf(red[tid],red[tid+r]); __syncthreads();} mx=red[0];
  float sum=0; for(int t=tid;t<CTX;t+=128){float e=expf(scores[t]-mx); scores[t]=e; sum+=e;} red[tid]=sum; __syncthreads();
  for(int r=64;r;r>>=1){if(tid<r)red[tid]+=red[tid+r]; __syncthreads();} float inv=1.f/red[0];
...
__global__ void logits_ht(const __half* __restrict__ Wt,const __half* __restrict__ x,float* __restrict__ y,int K,int N){
  int lane=threadIdx.x&31, warp=threadIdx.x>>5, row=blockIdx.x*8+warp; if(row>=N) return;
  const __half2* w2=reinterpret_cast<const __half2*>(Wt+(size_t)row*K); const __half2* x2=reinterpret_cast<const __half2*>(x); int K2=K>>1; __half2 acc2=__float2half2_rn(0.f);
  for(int k=lane;k<K2;k+=32){acc2=__hfma2(w2[k],x2[k],acc2);}
  float acc=__half2float(acc2.x)+__half2float(acc2.y); for(int off=16;off;off>>=1) acc += __shfl_down_sync(0xffffffff, acc, off);
